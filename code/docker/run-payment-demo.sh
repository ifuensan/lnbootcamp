...
run-in-node Chan "cli listpeers | jq -e '.peers[] | select(.id == \"${dina_address}\")' > /dev/null" \
&& {
...
wait-for-node Chan "cli listchannels | jq -e '.channels[] | select(.destination == \"${dina_address}\" and .active == true)'"
